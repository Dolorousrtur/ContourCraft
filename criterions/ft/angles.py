...
class Criterion(nn.Module):
    ...
    def calc_single(self, example):
        ...
        target_pos = example['cloth'].target_pos
        ...
        fn_rest = self.face_normals_f(target_pos.unsqueeze(0), faces.unsqueeze(0))[0]
        n_rest = gather(fn_rest, f_connectivity, 0, 1, 1)
        n0_rest, n1_rest = torch.unbind(n_rest, dim=-2)

        v_rest = gather(target_pos, f_connectivity_edges, 0, 1, 1)
