...
from utils.cloth_and_material import FaceNormals, ClothPYGCacheMatAug
...
class TrainingModule(nn.Module):
    def __init__(self, model: nn.Module, criterion_dict: Dict[str, nn.Module],
        ...
        self.model = model
        self.criterion_dict_hood = criterion_dict['criterions_hood']
        self.criterion_dict_impulse = criterion_dict['criterions_impulse']
        self.mcfg = mcfg

        self.cloth_obj = ClothPYGCacheMatAug(None, always_overwrite_mass=True)
        ...
        self.safecheck = self.mcfg.enable_repulsions and not self.mcfg.enable_attractions
    ...
    def _rollout(self, sample, start_idx, n_steps, progressbar=False, bare=False, safecheck=True):
        ...
        for i in pbar:
            ...
            if i > 0 and safecheck:
                step_check = self.check_step(sample_step)
                # print('step_check', step_check)
                fail_reason = 'PREV_CHECK'
                if not step_check:
            ...
            with torch.no_grad():
                with TorchTimer(metrics_dict, 'hood_time', start=start, end=end):
                    sample_step = self.model(sample_step, is_training=False, fake_icontour=True)

            prev_out_sample = sample_step.detach()

            if safecheck:
                if not self.check_step(prev_out_sample, prev=False, threshold=self.mcfg.safecheck.riz_epsilon):

                    with TorchTimer(metrics_dict, 'safcheck_impulse_time', start=start, end=end):
                        prev_out_sample, metrics_dict, valid = self.safecheck_impulse_pass(prev_out_sample,
                                                                                           metrics_dict)
                    if not valid:
                        fail_reason = 'IMPULSE_NAN'
                        print('Impulse_NAN')
                        break

                    if not self.check_step(prev_out_sample, prev=False, threshold=self.mcfg.safecheck.riz_epsilon):

                        with TorchTimer(metrics_dict, 'safcheck_RIZ_time', start=start, end=end):
                            prev_out_sample, metrics_dict, valid = self.safecheck_riz_pass(prev_out_sample,
                                                                                           metrics_dict)
                        if not valid:
                            fail_reason = 'RIZ_NAN'
                            print('RIZ_NAN')
                        ...
                        metrics_dict['safcheck_RIZ_time'].append(0.)
                        metrics_dict['riz_iters'].append(0.)
                        metrics_dict['riz_itersmax_riz_size'].append(0.)
                        pass
                        # print('no col after impulse')
                else:
                    metrics_dict['safcheck_impulse_time'].append(0.)
                    metrics_dict['safcheck_RIZ_time'].append(0.)
                    metrics_dict['impulse_iters'].append(0)
                    metrics_dict['impulse_stencil_ncoll'].append(0)
                    metrics_dict['riz_iters'].append(0)
                    metrics_dict['riz_itersmax_riz_size'].append(0)
                    pass
                    # print('no col after step')
            ...
            if not bare:
                loss_dict_hood, loss_weight_dict_hood, _ = self.criterion_pass(sample_step, self.criterion_dict_hood)
    ...
    def criterion_pass(self, sample_step, criterion_dict):
        ...
        return loss_dict, loss_weight_dict, metrics_dict
    ...
    def optimizer_step(self, optimizer, scheduler, loss_dict):
    ...
    def forward_short(self, sample, roll_steps=1, optimizer=None, scheduler=None) -> dict:
        ...
        for i in range(roll_steps):
            ...
            loss_dict_hood, loss_weight_dict_hood, loss_metrics_dict = self.criterion_pass(sample_step, self.criterion_dict_hood)
            ...
            self.optimizer_step(optimizer, scheduler, loss_dict_hood)
...
def create_optimizer(training_module: TrainingModule, mcfg: DictConfig):
...
def run_epoch(training_module: TrainingModule, aux_modules: dict, dataloader_short: DataLoader,
